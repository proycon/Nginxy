...
$('td a').each(function(){
	...
	for (var i = 0; i < formats.length; i++) {
		if (fileExt.toLowerCase() == formats[i].toLowerCase()) {
			...
			$(this).html('<img class="icons" src="/.nginxy-elke/images/icons/' + formats[i] + '.png" style="margin:0px 4px -4px 0px"></img></a>' + oldText);
			return;
		}
	}
	...
	if ($(this).text().indexOf("Parent directory") >= 0) {
		var found = 1;
		var oldText = $(this).text();
		$(this).html('<img class="icons" src="/.nginxy-elke/images/icons/home.png" style="margin:0px 4px -4px 0px">' + oldText);
		return;
	}
	...
	if ($(this).attr('href').substr($(this).attr('href').length - 1) == '/') {
		var found = 1;
		var oldText = $(this).text();
		$(this).html('<img class="icons" src="/.nginxy-elke/images/icons/folder.png" style="margin:0px 4px -4px 0px">' + oldText.substring(0, oldText.length - 1));
	...
	if (found == 0){
		var oldText = $(this).text();
		$(this).html('<img class="icons" src="/.nginxy-elke/images/icons/error.png" style="margin:0px 4px -4px 0px">' + oldText);
		return;
	}
});
});
